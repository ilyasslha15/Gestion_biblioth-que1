...
function updateLinksTextFR() {
    if (user.role !== "admin") {
        document.querySelectorAll("a")[3].textContent = "Lister livres";
    } else {
        document.querySelectorAll("a")[3].textContent = "Ajouter livre";
...
function updateLinksTextEN() {
    if (user.role !== "admin") {
        document.querySelectorAll("a")[3].textContent = "List books";
    } else {
        document.querySelectorAll("a")[3].textContent = "Add book";
...
function updateOverviewTextEN() {
    const titles = ["OverView", "Totale Revenue", "Number Of the Books", "Revenue Growth (%)"];
    document.querySelectorAll("h5").forEach((h, i) => h.textContent = titles[i]);
}
